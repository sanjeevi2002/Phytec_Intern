...
osThreadId tx_taskHandle;
osThreadId rx_taskHandle;
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* definition and creation of tx_task */
  osThreadDef(tx_task, Transmit, osPriorityNormal, 0, 128);
  tx_taskHandle = osThreadCreate(osThread(tx_task), NULL);

  /* definition and creation of rx_task */
  osThreadDef(rx_task, Receive, osPriorityIdle, 0, 128);
  rx_taskHandle = osThreadCreate(osThread(rx_task), NULL);
